...
class PyQuestFrame(wx.Frame):
    def __init__(self,parent,obj_id):
        ...
        self.sizer.Add(self.run_panel,0,wx.EXPAND)
        self.sizer.Add(self.notebook_panel,1,wx.EXPAND)
...
class PyQuestRunPanel(wx.Panel):
    def __init__(self,parent):
        ...
        self.loaded_data_box = wx.StaticText(self,wx.ID_ANY,
        ...
        self.hsizer.Add(self.t_sizer1,0,flag=wx.ALL,border=10)
        self.hsizer.Add(self.t_sizer2,0,flag=wx.ALL,border=10)
        self.hsizer.Add(self.t_sizer3,0,flag=wx.ALL,border=10)
        
        self.sizer.Add(self.loaded_data_box,0,flag=wx.ALL,border=10)
    ...
    def on_data_load(self,message):
        self.loaded_data_box.SetLabel("Data file loaded: {}".format(global_model.path))
        self.Refresh()
...
class PyQuestTreePage(wx.Panel):
    ...
    def recreate(self):
        self.sizer = wx.BoxSizer(wx.VERTICAL)
        self.sizer.Add(self.tree_plot,5,wx.EXPAND)
    ...
    def calc_edgecolors(self):
        ...
        if self.folder_checkbox.GetValue():
            ...
            for idx in t.elements:
                edgecolors[idx] = "k"
        self.embed_plot.update(edgecolors=edgecolors)
...
class EmbedPlot(wxplot_util.PlotPanel):
    def __init__(self,parent,obj_id,vecs,vals,x_vec=1,y_vec=2,z_vec=3,diff_t=1,
                 c=None,tree_level=1,embed_title="Embedding",edgecolors='none'):
        wxplot_util.PlotPanel.__init__(self,parent,obj_id)
        self.update(vecs=vecs,vals=vals,x_vec=x_vec,y_vec=y_vec,z_vec=z_vec,
                    diff_t=diff_t,c=None,tree_level=tree_level,
    ...
    def draw(self):
        ...
        self.ax.scatter3D(x,y,z,c=self.c,cmap=cmap,norm=cnorm,
                          edgecolors=self.edgecolors,linewidth=5)
